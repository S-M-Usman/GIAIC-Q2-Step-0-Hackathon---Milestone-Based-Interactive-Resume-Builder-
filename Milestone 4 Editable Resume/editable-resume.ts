...
class EditableResume {
  private editableSections: EditableSections = {};
  ...
  private initializeEditableSections(): void {
  ...
  private initializeDynamicSection(containerId: string): void {
  ...
  private makeEditable(sectionId: string): void {
  ...
  private startEditing(sectionId: string): void {
    const section = this.editableSections[sectionId];
    if (!section || section.isEditing) return;
  ...
  private updateFormField(sectionId: string, value: string): void {
    const formFieldMap: FormFieldMap = {
    ...
    if (formFieldClass) {
      ...
      if (formField) {
        ...
        const generateCV = (window as any).generateCV;
  ...
  private addNewItem(containerId: string): void {
    const container = document.getElementById(containerId);
    if (!container) return;
  ...
  private setupPreviewSection(): void {
...
document.addEventListener("DOMContentLoaded", () => {
  (window as any).editableResume = new EditableResume();
});
